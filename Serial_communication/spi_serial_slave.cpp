 ...
 * Author : Buddy
  ...
  This program sets up SPI in Slave mode, mode 0, with SCK frequency of Fosc/16.
    This is the counterpart to the SPI_master.cpp code
    Then data is transmitted and received.
	
  SPI is operating in full duplex, so data can go out and in from/to Master/Slave.
...
int main(void)
{
  ...
  DDRD = 0xFF; // Set DDRD to display data that comes in from Master
  // set up SPI I/O
  ...
  // enable the SPI, set to Slave mode 0, SCK = Fosc/16, lead with MSB
  ...
  SPDR = 'N';  // Move data to be transmitted to SPDR.  In this example, the letter N (Note this is interpreted as the ASCII code: 'N' = 0x4E)
	// Data transfer from slave does not begin until it detects clock from master
