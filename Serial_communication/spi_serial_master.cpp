 ...
 * Author : Buddy
   ...
   This program sets up SPI in Master mode, mode 0, with SCK frequency of Fosc/16.
   Then the letter 'R' is transmitted.
   Whatever data that is received from Slave is displayed on PortD
 ;
...
int main(void) {
	...
	// setup
	DDRD = 0xFF; // Set DDRD to display data that comes in from Slave
	...
	SPCR = 0b01010001; // SPCR = 1<<SPE | 1<<MSTR | 1<<SPR0; // (SPIE = 0, SPE = 1, DORD = 0, MSTR = 1, CPOL = 0, CPHA = 0, SPR1 = 0, SPR0 = 1)
		// enable the SPI, set to Master mode 0, SCK = Fosc/16, lead with MSB
		
	// Transmit the data
	PORTB &= ~(0b00000100); //(1 << PORTB2);  // Clear the SS bit to enable slave
	...
	PORTB |= 0b00000100; //PORTB |= 1 << PORTB2;  // disable slave (this clears SBIF flag)
